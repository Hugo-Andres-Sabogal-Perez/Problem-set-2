...
base_educ <- base_educ %>%
  mutate(maxedu_str = recode(maxedu,
...
continuas <- c(
  "num_cuartos", "num_dormitorios", "Nper", "Npersug", "pmujer", "nninos",
...
# Testeo:
continuas_t <-c('P5000', 'P5010', 'Nper', 'Npersug', 
              'pmujer', 'nninos', 'nviejos','P6426', 'P6800')

TH <- TH %>% mutate_at(continuas_t, ~ (scale(.) %>% as.vector()))
...
# Testeo:
TH <- TH %>% mutate_at(continuas_t, ~ (ifelse((.) >= 2.5, 2.5, (.))))
TH <- TH %>% mutate_at(continuas_t, ~ (ifelse((.) <= -2.5, -2.5, (.))))
...
# Entrenamiento:
categoricas <- c(
  "Dominio", "Clase.x", "P5090", "Depto", "maxedu", "Oficio",
